...
// Each task should have an unique ID. This is necessary to  specify
// which task should be woken
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
struct TaskId(u64);

impl TaskId {
    fn new() -> Self {
        // Use AtomicU64 to make sure each ID is only assigned once
        static NEXT_ID: AtomicU64 = AtomicU64::new(0);
        // The fetch_add method atomically increases the value and returns the previous value
        // in one atomic operation. This means that even when the TaskId::new method is called
        // in parallel, every ID is returned exactly once
